        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <a href='/'>Home</a>>
                    {{ $GLOBALS['currentBerry']->name }}
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-sm-12 col-md">
                            <h2>Colours for this berry</h2>
                            <ul>
                            <?php
                                foreach($GLOBALS['currentBerry']->getColorTags() as $tag)
                                {
                                    echo "<li>\n";
                                    echo $tag;
                                    echo "</li>\n";
                        ...
                        <div class="col-sm-12 col-md">
                            <h2>Tastes for this berry</h2>
                            <ul>
                            <?php
                                foreach($GLOBALS['currentBerry']->getTasteTags() as $tag)
                                {
                                    echo "<li>\n";
                                    echo $tag;
                                    echo "</li>\n";
                        ...
                        <div class="col-sm-12 col-md">
                            <h2>Uses for this berry</h2>
                            <ul>
                            <?php
                                foreach($GLOBALS['currentBerry']->getUsesTags() as $tag)
                                {
                                    echo "<li>\n";
                                    echo $tag;
                                    echo "</li>\n";
                    ...
                    <div id="berry-container" class="ibga-display-container">
                        ...
                        if(App\Variety::getNumberOfVarietiesWithAuctions($GLOBALS['currentBerry']->id) > 0)
                        {
                            echo "<h2>Narrow your search by variety!</h2>";
                            echo "<div id='berry-box-display' class='clearfix'> </div>";
                            echo "<button id='load-varieties' type='button' class='ibga-load-button btn btn-primary btn-block' onclick='loadVarieties()'>Load More</button>";
                        }
                        else
                        {
                            echo "<h2>Unfourtunately there are no varieties of this berry available right now.</h2>";
                        }
                    ...
                    <div id="auction-container" class="ibga-display-container">
                        ...
                        if(App\Auction::getAuctionCountByBerry($GLOBALS['currentBerry']->id) > 0)
                        {
...
@section('script')
<script>
    function checkForMoreVarieties(){
        // Check that there are still berries unloaded
        $.get("/get-current-varieties-count?berry={{ $GLOBALS['currentBerry'] }}", { }, function(data){
            var totalVarieties = data;
            // If enough berries have been loaded, hide the button
            document.getElementById('load-varieties').style.display = ((varietiesLoaded >= totalVarieties) ? 'none' : 'block');
        }, "json");
    }
    function loadVarieties(){
        // Load more berries to the view
        $.get("/get-current-varieties", {berry:loadedBerry, position:varietiesLoaded, count:4 }, function(data){
            var box = document.getElementById('berry-box-display');
            for(var i = 0; i < data.length; i++){
                var link = document.createElement("a");
                link.setAttribute("class", 'ibga-display-box rounded');
                link.setAttribute("href", "{{ route('varietyAuctions', '') }}" + "/" + data[i].name.toLowerCase());
                ...
                $.get("/get-variety-image", {variety:data[i].id}, function(data){
            ...
            varietiesLoaded += data.length;
            // Hide load button if needed
            checkForMoreVarieties();
        }, "json");
    }
    function checkForMoreAuctions(){
        // Check that there are still auctions unloaded
        $.get("/get-auction-count-by-berry?berry=${loadedBerry}", { }, function(data){
    ...
    function loadAuctions(){
        ...
        $.get("/get-auctions-by-berry", { berry:loadedBerry, position:auctionsLoaded, count:4 }, function(data){
    ...
    var loadedBerry = {{ $GLOBALS['currentBerry']->id }};
    var varietiesLoaded = 0;
